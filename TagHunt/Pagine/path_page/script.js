...
document.addEventListener("DOMContentLoaded", function () {
  fetch("../../tutto.json")
    ...
    .then((data) => {
      if (!localStorage.getItem("tappe")) {
        ...
        console.log(
          "JSON gia' caricato in localStorage:",
          json.parse(localStorage.getItem("tappe"))
        );
      }
    })
  ...
  document.querySelectorAll(".step").forEach((step) => {
    ...
    if (userData[userid].steps[id - 1].completed == true) {
      ...
      stepContainer.querySelector(".step-tag").textContent =
        userData[userid].steps[id - 1].tag;
    }
  });
});
  ...
  step.addEventListener("mouseenter", function () {
    ...
    if (triangle && triangle.classList.contains("triangle")) {
      ...
      triangle.style.animation =
        "bouncePersonal 1s infinite alternate ease-in-out";
    }
